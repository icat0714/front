      ...
      <el-table highlight-current-row :data="tableData" border style="width: 100%">
        ...
        <el-table-column prop="treatmentstate" label="返货处理状态">
          ...
          <template slot-scope="scope">
            <span v-if="tableData[scope.$index].treatmentstate==1"><el-tag type="danger">未确定</el-tag></span>
            <span v-if="tableData[scope.$index].treatmentstate==2"><el-tag type="success">同意返货</el-tag></span>
            <span v-if="tableData[scope.$index].treatmentstate==3"><el-tag type="warning">拒绝</el-tag></span>
            <span v-if="tableData[scope.$index].treatmentstate==4"><el-tag type="success">同意转发</el-tag></span>
          </template>
        </el-table-column>
        <el-table-column prop="identificationsign" label="系统自动确认标志">
          <template slot-scope="scope">
            <span v-if="tableData[scope.$index].identificationsign==1"><el-tag type="danger">未确定</el-tag></span>
            <span v-if="tableData[scope.$index].identificationsign==0"><el-tag type="success">已确定</el-tag></span>
          </template>
        </el-table-column>
        <el-table-column prop="address" label="返货信息">
        </el-table-column>
